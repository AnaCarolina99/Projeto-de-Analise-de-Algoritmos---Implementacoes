...
// Usando o algoritmo de Bellman-Ford ele descobre o menor caminho detectando se existe um ciclo negativo.
bool isNegative(struct Grafo* grafo, int vertice) { 
	int V = grafo->V; 
	int A = grafo->A; 
	int dist[V]; 
...
int main() { 
  ...
  start = clock(); 
	...
	if (isNegative(grafo, 0)) 
		printf("O grafo possui ciclo negativo"); 
	else
		printf("O grafo não possui ciclo negativo");  
